...
with tab_doc:
    ...
    The synthetic banking dataset was generated using a Python-based pipeline that combined real human typing behaviour from the KeyRecs keystroke dataset with simulated banking activity.

    1. KeyRecs behavioural feature extraction:
       The KeyRecs keystroke data was processed to generate participant-level behavioural profiles. Features included dwell time, flight/inter-key time, typing speed, typing variability, and total typing duration. The DU, DD, UD, and UU timing measurements were used to capture different aspects of individual typing rhythm.

    2. Behavioural profile cleaning:
       The extracted profiles were validated for invalid and extreme timing values. Profiles containing unsuitable timing data were removed, leaving a cleaned set of behavioural profiles for synthetic-user generation.

    3. Synthetic user generation:
       The cleaned KeyRecs profiles were used as templates for generating 500 synthetic banking users. Python's sampling functionality was used to select behavioural profiles with replacement, using random_state=42 for reproducibility. Each generated user was assigned a unique ID from U0001 to U0500. Controlled random variation was applied to the sampled behavioural characteristics so that users derived from the same KeyRecs profile were not identical.

    4. Banking session generation:
       Normal banking sessions were generated for the synthetic users using their assigned behavioural profiles. Each session included characteristics such as typing behaviour, device familiarity, location familiarity, time of activity, screens visited, paste usage, and session duration.

    5. Transaction generation:
       Banking transactions were generated and associated with the users' sessions. Transaction attributes included transaction amount, deviation from the user's typical amount, recipient information, time since the previous transaction, and transaction frequency.

    6. ATO activity generation:
       Account-takeover sessions and transactions were generated by modifying behavioural and contextual patterns within existing user accounts. The simulated ATO activity introduced combinations of changes such as unfamiliar devices, unfamiliar locations, unusual typing behaviour, and new recipients, while avoiding uniformly extreme values.

    7. Dataset combination and validation:
       The legitimate and ATO activity was combined into the final dataset and assigned corresponding labels. The resulting data contains 500 synthetic users, 21,500 sessions, and 24,455 transactions. The final model dataset contains 19 columns and was validated for consistency and missing values before model development.
